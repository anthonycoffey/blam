...
> **Audience.** AI agents about to write code in `Woop/` or author a new built-in/custom script. Read this first.
...
This is a fork of upstream [Woop](https://github.com/felixse/Woop), itself a Windows port of macOS [Boop](https://github.com/IvanMathy/Boop). Scripts are sourced from the `submodules/Boop` git submodule, copied into `Woop/Assets/Scripts/` on every build.
...
| Test framework | MSTest (UWP Unit Test App) — see `Woop.Tests/` |
...
- **App entry:** `Woop/App.xaml.cs` → `Woop.App` class.
- **Main UI:** `Woop/Views/MainPage.xaml(.cs)` bound to `Woop/ViewModels/MainViewModel.cs`.
- **Script execution:** `Woop/Models/Script.cs::Run()` invokes user JavaScript inside a per-script `V8ScriptEngine`.
...
This is what JS scripts running inside Blam! see. The C# `ScriptExecution` object (`Woop/Models/ScriptExecution.cs`) is exposed to the V8 engine and is the entire contract.
...
| `icon` | recommended | Name of a PNG in `Woop/Assets/{dark,light}/` (`flask`, `dice`, `globe`, etc.) |
| `bias` | optional | Float that nudges fuzzy ranking; positive = surface earlier |
| `author` | optional | Credit string |

Metadata is parsed from the `/** ... **/` block at the top of the script via `JsonSerializer.Deserialize<ScriptMetadata>` with `AllowTrailingCommas = true, PropertyNameCaseInsensitive = true`. See `Woop/Models/Script.cs`.

## Where scripts live

| Location | Source of truth | Build behavior |
|---|---|---|
| `Woop/Assets/Scripts/` | **None — gitignored** | Wiped + repopulated from the submodule on every build by the pre-build event in `Woop.csproj` |
...
**Implication:** you cannot ship a Blam!-only built-in script by adding a file to `Woop/Assets/Scripts/` — it will be deleted on the next build. To ship a custom built-in script, either upstream it to Boop or modify the pre-build event to merge from a second source. The latter requires a spec.

## Settings

Persisted via `Windows.Storage.ApplicationData.Current.LocalSettings` (per-user, per-install). See `Woop/Services/SettingsService.cs`. Fields include theme override and custom scripts folder path.

## How to run locally

```powershell
git submodule update --init --recursive
# Open Woop.sln in Visual Studio 2019/2022 (UWP workload installed)
# Press F5
```
...
# Or:
msbuild Woop.sln /t:Restore /p:Configuration=Debug /p:Platform=x86
msbuild Woop.sln /t:Build   /p:Configuration=Debug /p:Platform=x86
vstest.console.exe Woop.Tests\bin\x86\Debug\Woop.Tests.build.appxrecipe
```
...
- **Pre-build deletes scripts.** `Woop.csproj` PreBuildEvent runs `del /q` then `xcopy` on `Assets/Scripts/`. Anything you drop into that folder by hand is destroyed at next build.
...
- **Cert thumbprint is upstream-owned.** `Woop.csproj` references `9B8BE8375019C354A32D6EFACC0808A7003F2432` belonging to "FS Apps" (Felix Seidl). Local sideload builds need either your own self-signed cert or `AppxPackageSigningEnabled=False` for unsigned builds.
...
- **Bundle platforms mismatch.** `Woop.csproj` `AppxBundlePlatforms=x86|x64|arm64` (no 32-bit ARM) even though Debug/Release configs exist for ARM. Sideload bundles include x86/x64/arm64 only.
- **Display name still says "Woop!"** The repo was renamed Blam! but `Package.appxmanifest` `DisplayName="Woop!"`. Out of scope for casual changes — a rename touches the solution, namespace, manifest, and Store identity together.
