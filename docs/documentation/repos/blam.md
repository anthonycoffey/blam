...
`Blam.csproj` ships with an empty `<PackageCertificateThumbprint>` — the upstream maintainer's thumbprint was scrubbed during the Woop → Blam port. A fresh clone has no signing cert configured. Options:
...
2. **Local sideload, self-signed:** generate via `New-SelfSignedCertificate -Type CodeSigningCert -Subject "CN=<your-name>"` and either install it to `Cert:\CurrentUser\My` then set the thumbprint in the csproj, or sign post-build with `signtool.exe`.
3. **Store submission:** use the Partner Center–issued publisher cert. The `REPLACE_*` placeholders in `Package.appxmanifest` (Identity Name, Publisher, PhoneProductId, PublisherDisplayName) must be filled in before submitting.
